...
/* Arrow Keys (extended PS/2 scancodes, preceded by 0xE0) */
    ...
    while (1) {
        ...
        if (b == KEY_M) {
            if (menu_state != MENU_CLOSED) {
                /* M always closes the menu fully */
        ...
        if (menu_state == MENU_MAIN) {
            if (b == KEY_1) {
                /* Enter instrument sub-menu */
                menu_state = MENU_INSTRUMENT;
                g_drawing_ui = 1;
                draw_instrument_menu();
                g_drawing_ui = 0;
            } else if (b == KEY_2) {
                /* Add a Page — not yet implemented */
                /* (do nothing) */
            } else if (b == KEY_3) {
                /* Go Back — close menu */
                menu_state = MENU_CLOSED;
        ...
        if (got_extended && (b == KEY_LEFT || b == KEY_RIGHT)) {
            int new_col = cur_col;
            if (b == KEY_LEFT  && cur_col > 1)              new_col--;
            if (b == KEY_RIGHT && cur_col < TOTAL_COLS - 1) new_col++;
            if (new_col != cur_col) {
